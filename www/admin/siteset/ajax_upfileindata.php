...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
 if($ajax_onuserperm_addprem=='1'){
   ...
   $filetag= $_REQUEST["filetag"];//
   $upfiletype = strtoupper($_REQUEST["upfiletype"]);//上文件类型
   $strSQL="INSERT INTO layoutpic(id_hr,opicname,id_layout,type,indate,url,title,intro,tag)
	                     values('$_SESSION[user_id]','$filename','$upid','$upfiletype',now(),'$filetitleurl','$filetitle','$filejj','$filetag')";
