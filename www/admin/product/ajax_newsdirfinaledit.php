...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
  if($ajax_onuserperm_editprem=='1'){
   ...
   if ($isdel=='1' && $ajax_onuserperm_deleprem=='1'){
   	$strSQL="update productdir3 set dele='0' where id_proddir='".$newsid."' and id_proddir not in (1)";
	$objDB->Execute($strSQL);
   }else{
   
    if($newsid==1){
     $strSQL="UPDATE productdir3 SET fatherid='".$fatherid."',name='".$newsdir2name."',ordernum='".$menuorder."',lang='".$lang."',intro='".$intro."' where id_proddir='".$newsid."'";
     $objDB->Execute($strSQL);
	 //更新所有下级目录语言为父目录语言
	 $strSQL="UPDATE productdir3 SET lang='".$lang."' where fatherid='".$newsid."' and level=2";
     $objDB->Execute($strSQL);
	 
   }else{
   $strSQL="UPDATE productdir3 SET fatherid='".$fatherid."',name='".$newsdir2name."',intro='".$intro."',ordernum='".$menuorder."',lang='".$lang."' where id_proddir='".$newsid."'";
   $objDB->Execute($strSQL);
   	 //更新所有下级目录语言为父目录语言
	 $strSQL="UPDATE productdir3 SET lang='".$lang."' where fatherid='".$newsid."' and level=2 ";
     $objDB->Execute($strSQL);
