...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
  ...
  //查出父目录语言项目
  $strSQL="SELECT lang FROM productdir3 WHERE id_proddir='".$val2."' ";
  $objDB->Execute($strSQL);
  $onefatherlang=$objDB->fields();
  
  $strSQL="INSERT INTO productdir3(name,level,fatherid) VALUES('".$val1."','2','".$val2."') ";
  $objDB->Execute($strSQL);//2级菜单名称入库
  
  $id = $objDB->getInsertID();//获取刚入库记录的ID
  $strSQL="update productdir3 set dirtree='".$val2.",".$id."',ordernum='".$id."',lang='".$onefatherlang[lang]."' where id_proddir='".$id."'";
