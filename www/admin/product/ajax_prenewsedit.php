...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
  if($ajax_onuserperm_editprem=='1'){
   ...
   //取出fatherid信息
   $strSQL="SELECT fatherid,name,id_proddir,intro FROM productdir3 WHERE  id_proddir=$proddirid and  dele='1' order by ordernum ASC";
   $objDB->Execute($strSQL);
   $newsid=$objDB->fields();
   
   //取出一级目录
  $strSQL="SELECT id_proddir,name FROM productdir3 WHERE level='1' and dele='1' order by ordernum ASC";
  $objDB->Execute($strSQL);
