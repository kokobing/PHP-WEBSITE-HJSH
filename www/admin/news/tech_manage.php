...
if(isset($_GET[Rows])){//SESSION记忆行数
	...
	header('Location:tech_manage.php');ob_end_flush(); exit();//跳转
}
...
//交换位置
	if(isset($_GET["action"]) && $_GET["action"]=="switch"){
		...
		$objDB->Execute($strSQL);
	    header('Location:tech_manage.php?&searchselect='.$_GET[searchselect].'&searchdate=&keywords=&page='.$_GET[page]); exit();//跳转
	}
...
//输出一级目录  
  $strSQL="SELECT id_newsdir,name FROM newsdir where dele='1' and level='1' and  (id_newsdir='42' or id_newsdir='44'  or id_newsdir='46' or id_newsdir='79' or id_newsdir='81' or id_newsdir='83' ) ";
  $objDB->Execute($strSQL);
...
$strSQLNum = "SELECT COUNT(*) as num from newsinfo left join newsdir on newsinfo.id_newsdir=newsdir.id_newsdir WHERE newsinfo.dele='1'  and (newsdir.fatherid='42' or newsdir.fatherid='44' or newsdir.fatherid='46' or newsdir.fatherid='79' or newsdir.fatherid='81' or newsdir.fatherid='83') $str";   
...
$strSQL = "SELECT newsinfo.id_newsinfo,newsdir.name,newsinfo.newsdate,newsinfo.title,newsinfo.content,newsinfo.ordernum,newsinfo.id_newsdir,newsdir.lang,newsdir.fatherid FROM newsinfo left join newsdir on newsinfo.id_newsdir=newsdir.id_newsdir WHERE newsinfo.dele='1'  and (newsdir.fatherid='42' or newsdir.fatherid='44' or newsdir.fatherid='46' or newsdir.fatherid='79' or newsdir.fatherid='81' or newsdir.fatherid='83') $str order by newsinfo.ordernum DESC" ;
...
//添加空白信息入库
 if(isset($action) && $action=='01' && $onuserperm_addprem=='1'){
    ...
    header('Location:tech_manage.php'); exit();
}	
//编辑信息入库
if(isset($action) && substr($action,0,2)=="04" && $onuserperm_editprem=='1'){
	...
	$objDB->Execute($strSQL);
	header('Location:tech_manage.php?03'.$onenewsinfoid.'&searchselect='.$searchselect.'&searchdate='.$searchdate.'&keywords='.$keywords.'&modify=1'); exit();
}	
  ...
  <tr> 
    ...
    <td width="75%" align="center" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <a href="tech_manage.php?01" class="txt_addnews" >添加信息</a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <?php }?>
        </td>
      </tr>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="77%" height="36" align="left" background="../inc/pics/lanmubg.gif"><table width="200" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="153" class="txt_lanmu">Inspiration</td>
            </tr>
          </table></td>
          <td width="23%" align="center" background="../inc/pics/lanmubg.gif"  class="txt">每页显示 <a href="tech_manage.php?Rows=5" class="link_rightmenu">5</a> <a href="tech_manage.php?Rows=10"  class="link_rightmenu">10</a> <a href="tech_manage.php?Rows=15" class="link_rightmenu">15</a> <a href="tech_manage.php?Rows=20" class="link_rightmenu">20</a> <a href="tech_manage.php?Rows=25" class="link_rightmenu">25</a> <a href="tech_manage.php?Rows=30" class="link_rightmenu">30</a></td>
        </tr>
        <tr>
          <td colspan="2"></td>
        </tr>
          ...
          <td colspan="2" align="center">
              ...
              <tr>
                ...
                <!--<?php for($i=0;$i<sizeof($arr2dirsearch2);$i++){?>
				    <a href="tech_manage.php?&searchselect=<?php echo $arr2dirsearch2[$i][id_newsdir];?>" <? if($_GET[searchselect]==$arr2dirsearch2[$i][id_newsdir]){echo 'class="link_dir2s"';}else{echo 'class="link_dir2"';}?> ><?php echo $arr2dirsearch2[$i][name];?></a> <span class="txt_footer">|</span> 
				<?php }?>   
                <br>
                <?php for($i=0;$i<sizeof($arr2dirsearch);$i++){?>
				    <a href="tech_manage.php?&searchselect=<?php echo $arr2dirsearch[$i][id_newsdir];?>" <? if($_GET[searchselect]==$arr2dirsearch[$i][id_newsdir]){echo 'class="link_dir2s"';}else{echo 'class="link_dir2"';}?> ><?php echo $arr2dirsearch[$i][name];?></a> <span class="txt_footer">|</span> 
				<?php }?> 
                 --><br>
                </td>
              </tr>
        ...
        <tr>
          <td colspan="2" align="center"><table width="95%" border="0" cellspacing="0" cellpadding="0"  id="otable">
			     ...
			     $strSQL="SELECT name from newsdir where id_newsdir='".$arrnewsinfos[$i][fatherid]."' ";
                 $objDB->Execute($strSQL);
                 $dir1name=$objDB->fields();
            ...
            <tr>
              ...
              <table width="100%" border="0" cellspacing="5" cellpadding="3">
                  <tr>
                    <td align="left" width="37%" class="txt_leftmenu">[ <?php echo $dir1name[name]?> / <?php echo $arrnewsinfos[$i][name]?> ]</td>
                    <td width="11%" align="left" class="txt_leftmenu">[ <?php echo $arrnewsinfos[$i][newsdate]?> ]</td>
                    <td width="29%" align="left"><a href="tech_manage.php?03<?php echo $arrnewsinfos[$i][id_newsinfo];?>&searchselect=<?php echo $searchselect;?>&searchdate=<?php echo $searchdate;?>&keywords=<?php echo $keywords;?>"  class="txt_leftmenu"><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></a></td>
...
<a href='tech_manage.php?action=switch&id=<?=$arrnewsinfos[$i][id_newsinfo];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i-1][id_newsinfo];?>&swtorder=<?=$arrnewsinfos[$i-1][ordernum];?>&page=<?=$_GET[page];?>&searchselect=<?=$_GET[searchselect];?>'><img src='../inc/pics/icon_001.gif' border='0' width='10' height='10'></a>&nbsp;
			<? }else {?>
<img src='../inc/pics/icon_0012.gif' border='0' width='10' height='10'>&nbsp;
			 
			<? } if($i!=$intNews-1){//判断一级目录是否位置在最后?>
<a href='tech_manage.php?action=switch&id=<?=$arrnewsinfos[$i][id_newsinfo];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i+1][id_newsinfo];?>&swtorder=<?=$arrnewsinfos[$i+1][ordernum];?>&page=<?=$_GET[page];?>&searchselect=<?=$_GET[searchselect];?>'><img src='../inc/pics/icon_002.gif' border='0' width='10' height='10'></a>&nbsp;
			<? } else{ ?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td height="30" align="left" valign="bottom" class="txt_leftmenu">
              <form action="tech_manage.php?04<?php echo $onenewsinfo[id_newsinfo];?>&searchselect=<?php echo $searchselect;?>&searchdate=<?php echo $searchdate;?>&keywords=<?php echo $keywords;?>" method="post" name="form" id="form"  onsubmit="return OnCheck(this);" >
...
<?php  }}?>
                  ...
                  <tr bgcolor="#FFFFFF">
                    ...
                    <input class="btn" type="button" name="button_back2" id="button_back2" value="返回" onclick="javascript:location.href='tech_manage.php?&searchselect=<?php echo $searchselect;?>&searchdate=<?php echo $searchdate;?>&keywords=<?php echo $keywords;?>'" />                    </td>
                  </tr>
              </table>
              </form></td>
            </tr>
