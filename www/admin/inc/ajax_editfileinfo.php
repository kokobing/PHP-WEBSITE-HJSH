...
$getfiletag=$_REQUEST["getfiletag"];
...
if(empty($_FILES['fileupload']['tmp_name']) || $_FILES['fileupload']['tmp_name'] == 'none'){
   ...
   if($datasheet=='layoutpic'){
    $strSQL="UPDATE layoutpic SET title='$getfiletitle',intro='$getfileintro',url='$getfileurl',tag='$getfiletag' where id_layoutpic=$getfileid";
   ...
   $errorinfo = "信息修改完成！";
	...
	if(strtoupper($getfiletype)!=strtoupper($type)){
		  ...
		  if($datasheet=='layoutpic'){
		  $strSQL="UPDATE layoutpic SET title='$getfiletitle',intro='$getfileintro',url='$getfileurl',opicname='$upfilename',tag='$getfiletag' where id_layoutpic=$getfileid";
	      $objDB->Execute($strSQL);
		  }
